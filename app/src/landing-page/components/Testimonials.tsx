...
export default function Testimonials({ testimonials }: { testimonials: Testimonial[] }) {
  return (
    <div className='mx-auto mt-32 max-w-7xl px-6 py-4 sm:mt-56 sm:px-6 sm:py-0 lg:px-8'>
      <div className='relative sm:left-5 sm:-m-2 rounded-xl bg-yellow-400/20 lg:ring-1 lg:ring-yellow-500/50 lg:-m-4 '>
        <div className='relative top-4 right-4 sm:top-5 sm:right-5 bg-gray-900 dark:bg-boxdark px-8 py-12 shadow-xl rounded-xl sm:px-10 sm:py-16 md:px-12 lg:px-20'>
          ...
          <div className='relative flex flex-wrap gap-6 w-full mt-6 z-10 justify-between lg:mx-0'>
            {testimonials.map((testimonial, idx) => (
              <figure key={idx} className='w-full lg:w-1/4 box-content flex flex-col justify-between p-8 rounded-xl bg-gray-500/5 '>
                ...
                <figcaption className='mt-6 text-sm text-white sm:text-base'>
                  <a href={testimonial.socialUrl} className='flex items-center gap-x-2'>
                    <img src={testimonial.avatarSrc} loading='lazy' className='h-12 w-12 rounded-full object-cover' />
                    <div>
