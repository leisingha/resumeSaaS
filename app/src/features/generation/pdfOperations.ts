...
export const generateResumePdf: GenerateResumePdf<
  GeneratePdfPayload,
  PdfResult
> = async (args, context) => {
  if (!context.user) {
  ...
  if (htmlContent.length > 50000) {
    // 50KB limit for optimal performance
    console.warn(
      `[generateResumePdf] Large content detected for user ${context.user.id}: ${htmlContent.length} chars`
    );
  ...
  console.log(
    `[generateResumePdf] User ${context.user.id} requested PDF generation for content length: ${htmlContent.length} chars`
  );
  ...
  try {
    ...
    console.log(
      `[generateResumePdf] PDF generated successfully for user ${context.user.id}, size: ${pdfBuffer.length} bytes`
    );
    ...
    try {
      ...
      console.log(
        `[generateResumePdf] Base64 conversion successful - length: ${
          pdfBase64.length
        }, sample: ${pdfBase64.substring(0, 50)}...`
      );
    } catch (conversionError) {
      console.error(
        `[generateResumePdf] Base64 conversion failed for user ${context.user.id}:`,
        conversionError
      );
    ...
    if (!/^[A-Za-z0-9+/]/.test(pdfBase64)) {
      console.error(
        `[generateResumePdf] Invalid base64 start for user ${
          context.user.id
        }: ${pdfBase64.substring(0, 20)}`
      );
      throw new Error(
        "Generated base64 string does not start with valid base64 characters"
      );
    }
    ...
    console.log(
      `[generateResumePdf] PDF conversion successful - base64 length: ${pdfBase64.length} chars`
    );

    return result;
  } catch (error: any) {
    console.error(
      `[generateResumePdf] Error generating PDF for user ${context.user.id}:`,
      error
    );
    ...
    if (error.message?.includes("timeout")) {
      throw new HttpError(
        408,
        "PDF generation timed out. Please try again with shorter content."
      );
    } else if (error.message?.includes("Navigation failed")) {
      throw new HttpError(
        500,
        "Failed to load content for PDF generation. Please check your content format."
      );
    } else if (error.message?.includes("Protocol error")) {
      throw new HttpError(
        500,
        "Browser communication error during PDF generation. Please try again."
      );
    ...
    if (browser) {
      try {
        ...
        await browser.close();
        console.log(
          `[generateResumePdf] Browser closed for user ${context.user.id}`
        );
      } catch (closeError) {
        console.warn(
          `[generateResumePdf] Error closing browser for user ${context.user.id}:`,
          closeError
        );
        ...
        try {
          ...
          console.error(
            `[generateResumePdf] Failed to force close browser for user ${context.user.id}:`,
            forceCloseError
          );
        }
      }
