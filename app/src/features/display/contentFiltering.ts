 ...
 */
export function filterContentBySections(htmlContent: string, sections: Section[]): string {
  ...
  if (skillsSectionState && !skillsSectionState.visible) {
    const skillsH2 = Array.from(tempDiv.getElementsByTagName('h2')).find(h2 => 
      h2.textContent?.toLowerCase().includes('skills')
    );
  ...
  if (projectsSectionState && !projectsSectionState.visible) {
    const projectsH2 = Array.from(tempDiv.getElementsByTagName('h2')).find(h2 => 
  ...
  if (languagesSectionState && !languagesSectionState.visible) {
    const languagesH2 = Array.from(tempDiv.getElementsByTagName('h2')).find(h2 => 
      h2.textContent?.toLowerCase().includes('languages')
    );
...
export function logSectionVisibility(sections: Section[]): void {
  ...
  sections.forEach(section => {
    console.log(`- ${section.label} (${section.id}): ${section.visible ? 'visible' : 'hidden'}`);
  });
}