...
const ProfileForm = () => {
  ...
  const newStandardInputClass = "w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary";
  const newStandardInputWithIconClass = newStandardInputClass + " pl-11.5";
      ...
      <div className='flex flex-col gap-5.5 sm:flex-row'>
        <div className='w-full sm:w-1/2'>
          ...
          <div className='relative'>
            ...
            <input className={newStandardInputWithIconClass} type='text' name='fullName' id='fullName' placeholder='Full Name' value={formData.fullName} onChange={handleChange} />
          </div>
        </div>
        <div className='w-full sm:w-1/2'>
          <label className={labelClassName} htmlFor='phoneNumber'>Phone Number</label>
          <input className={newStandardInputClass} type='tel' name='phone' id='phoneNumber' placeholder='Phone Number' value={formData.phone} onChange={handleChange} />
        </div>
      </div>
      <div>
        ...
        <div className='relative'>
          ...
          <input className={newStandardInputWithIconClass} type='email' name='email' id='emailAddress' placeholder='Email Address' value={formData.email} onChange={handleChange} />
        </div>
      </div>
      <div>
        <label className={labelClassName} htmlFor='bio'>Professional Summary</label>
        <div className='relative'>
          ...
          <textarea className={newStandardInputWithIconClass + " min-h-[120px]"} name='bio' id='bio' rows={6} placeholder='Write your professional summary here' value={formData.bio} onChange={handleChange}></textarea>
        </div>
      </div>
      ...
      {formData.education.map((edu, index) => (
        <div key={edu.id} className='space-y-4 mb-4 relative'>
          {formData.education.length > 1 && (
            <button type="button" onClick={() => removeEducationEntry(index)} className="absolute top-0 right-0 text-red-500 hover:text-red-700 p-1">
              Remove
            </button>
          )}
          <div>
            <label htmlFor={`institution-${index}`} className={labelClassName}>Institution</label>
            <input type='text' name='institution' id={`institution-${index}`} value={edu.institution} onChange={(e) => handleChange(e, 'education', index)} className={newStandardInputClass} placeholder="Stark University"/>
          </div>
          <div className='flex flex-col sm:flex-row gap-4'>
            <div className="w-full sm:w-1/2">
              <label htmlFor={`degree-${index}`} className={labelClassName}>Degree/Certificate</label>
              <input type='text' name='degree' id={`degree-${index}`} value={edu.degree} onChange={(e) => handleChange(e, 'education', index)} className={newStandardInputClass} placeholder="B.S. in Iron Studies"/>
            </div>
            <div className="w-full sm:w-1/2">
              <label htmlFor={`graduationYear-${index}`} className={labelClassName}>Graduation Year</label>
              <input type='text' name='graduationYear' id={`graduationYear-${index}`} value={edu.graduationYear} onChange={(e) => handleChange(e, 'education', index)} className={newStandardInputClass} placeholder="2010"/>
            </div>
          </div>
        </div>
      ))}
      ...
      {formData.experience.map((exp, index) => (
        <div key={exp.id} className='space-y-4 mb-4 relative'>
          {formData.experience.length > 1 && (
            <button type="button" onClick={() => removeExperienceEntry(index)} className="absolute top-0 right-0 text-red-500 hover:text-red-700 p-1">
                Remove
            </button>
          )}
          <div>
            <label htmlFor={`company-${index}`} className={labelClassName}>Company</label>
            <input type='text' name='company' id={`company-${index}`} value={exp.company} onChange={(e) => handleChange(e, 'experience', index)} className={newStandardInputClass} placeholder="Stark Industries"/>
          </div>
          <div className='flex flex-col sm:flex-row gap-4'>
            <div className="w-full sm:w-2/3">
              <label htmlFor={`role-${index}`} className={labelClassName}>Role/Title</label>
              <input type='text' name='role' id={`role-${index}`} value={exp.role} onChange={(e) => handleChange(e, 'experience', index)} className={newStandardInputClass} placeholder="CEO"/>
            </div>
            <div className="w-full sm:w-1/3">
              <label htmlFor={`duration-${index}`} className={labelClassName}>Duration</label>
              <input type='text' name='duration' id={`duration-${index}`} value={exp.duration} onChange={(e) => handleChange(e, 'experience', index)} className={newStandardInputClass} placeholder="2008 - Present"/>
            </div>
          </div>
          <div>
            <label htmlFor={`description-${index}`} className={labelClassName}>Description/Responsibilities</label>
            <textarea name='description' id={`description-${index}`} rows={4} value={exp.description} onChange={(e) => handleChange(e, 'experience', index)} className={newStandardInputClass + " min-h-[100px]"} placeholder="Saving the world, inventing cool tech..."></textarea>
          </div>
        </div>
      ))}
