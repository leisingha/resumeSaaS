...
import type { CustomizationOptions } from '../../AppPage'; // Import the interface
...
const ResumeCustomizer: React.FC<ResumeCustomizerProps> = ({ options, onOptionsChange, part }) => {
  ...
  return (
    <div className='bg-white dark:bg-boxdark shadow-md p-6 rounded-sm'>
      <h2 className='text-lg font-semibold text-black dark:text-white mb-4'>
        {part === 'templateControls' ? 'Template Options' : 'Resume Details'}
      </h2>
      ...
      {part === 'detailControls' && (
        <>
            ...
            <div className="flex rounded-md border border-stroke dark:border-form-strokedark">
              {toneOptions.map((toneOption) => (
                <button
                  ...
                  type="button"
                  onClick={() => onOptionsChange({ ...options, tone: toneOption.value })}
                  className={`flex-1 py-3 px-4 text-center font-medium transition focus:outline-none
                    ${options.tone === toneOption.value
                      ? 'bg-primary text-white'
                      : 'bg-transparent text-black hover:bg-gray-2 dark:text-white dark:hover:bg-boxdark-2'}
                    ${toneOption.value === 'formal' ? 'rounded-l-md' : ''}
                    ${toneOption.value === 'casual' ? 'rounded-r-md' : ''}
                    ${toneOption.value !== 'casual' ? 'border-r border-stroke dark:border-form-strokedark' : ''}
                  `}
                >
                  {toneOption.label}
                </button>
          ...
          <p className='text-sm text-gray-600 dark:text-gray-400 mt-6'>
            More controls (font choices, section visibility) will be added later.
          </p>
        </>
      )}
    </div>
